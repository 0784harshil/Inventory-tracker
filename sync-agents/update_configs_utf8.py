...
with open('deploy-store-h/config.ini', 'w', encoding='utf-8') as f:
    f.write(CONTENT)
    
CONTENT_K = CONTENT.replace('STORE-H', 'STORE-K').replace('cresqlh', 'cresqlk')
with open('deploy-store-k/config.ini', 'w', encoding='utf-8') as f:
    f.write(CONTENT_K)

print("Deploy configs updated successfully (UTF-8).")
